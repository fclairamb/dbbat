...
**Component**: Version text in bottom-right corner of the application.
...
**Component**: Fixed badge in bottom-left corner of the application (demo mode only).
...
2. **VersionDisplay**: New fixed-position component showing version (all modes)
3. **DemoModeBadge**: New fixed-position component (demo mode only)
4. **LoginPage**: Conditional pre-fill of credentials + helper text
5. **DatabaseForm**: Read-only host field + pre-fill username/password with `demo`/`demo`
6. **UserList/UserDetail**: Hide delete option for admin in demo mode
7. **UserSettings**: Hide password change for admin in demo mode
