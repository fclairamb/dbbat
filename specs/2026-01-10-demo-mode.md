...
**Requirement**: Display the application version in the frontend UI (all modes, not just demo).

**Locations**:

| Location | Position | When Visible |
|----------|----------|--------------|
| Login screen | Bottom center of the page | Before login |
| Left sidebar menu | Bottom of the menu | After login |

**Specifications**:
- **Style**: Small, muted text
- **Text**: `v{version}` (e.g., "v1.0.0")
- **Tooltip on hover**: Full version info including commit hash (e.g., "v1.0.0 (abc123)")
- **Data source**: Fetched from `GET /api/v1/version` endpoint

#### 4.2 Demo Mode Badge

**Component**: Badge displayed in sidebar and login screen (demo mode only).
...
**Specifications**:
- **Position**: Bottom of sidebar (after login) / Bottom center of login screen (before login)
...
**Note**: Default credentials `demo`/`demo` match the sample database created on startup.
...
| Sample database | Name: `demo_db`, Host: `localhost`, Port: `5432`, Database: `demo`, User: `demo`, Password: `demo` |
| Sample grants | `connector` → `demo_db` (write, 10 years), `viewer` → `demo_db` (read, 10 years) |
...
1. **Context/Hook**: Fetch version info and run mode from version endpoint on app init
2. **LoginPage**: Add version display at bottom + demo mode badge (if demo) + pre-fill credentials
3. **Sidebar**: Add version display at bottom + demo mode badge (if demo)
4. **DatabaseForm**: Read-only host field + pre-fill username/password with `demo`/`demo`
