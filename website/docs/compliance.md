...
- **Tamper-evidence is detection, not prevention, and it has edges.** The
  ...
  **before 0.24** carry a head stamp that is a plain copy of the last
  ...
  last statements and rewriting the stamp to match needs no key. From 0.24 that
  stamp is a keyed MAC, like the captured result rows' always was. Existing
  stamps cannot be re-sealed (the key never enters the database), so from 0.24
  `dbbat audit verify --queries` reports each of those sessions as a **break**:
  its statements verify, but nothing vouches for its tail.
  `--allow-legacy-stamps` restores the old, weaker outcome for the duration of
  an upgrade from 0.23.x — counted as
  `sessions_with_legacy_forgeable_head_stamp` rather than broken — and is
  removed in 0.25. See
