...
| `DBB_QUERY_STORAGE_RETENTION`, `DBB_CONNECTION_RETENTION` and `DBB_DUMP_RETENTION` | **C1.2** — disposal of confidential information *(only in scope if you selected the Confidentiality category)* | Query history, captured result rows and local captures age out on a schedule you set, rather than accumulating forever. |
...
| [HMAC-chained records](/docs/features/audit-chain): each audit entry, each statement and each captured result row carries a MAC over its content plus the previous record's MAC, verified offline with `dbbat audit verify`; the key is HKDF-derived from `DBB_KEY` and never written to the database | **A.8.15 Logging** (log protection), **A.5.33 Protection of records** | A.8.15 requires logs to be protected against unauthorised alteration; the chain makes alteration, deletion and reordering detectable without the log ever leaving DBBat. Read access to the store is not enough to forge a record, and neither is write access — only the key is. It detects tampering rather than preventing it, and does not cover rows written before the chain anchor. Deletion from the *end* of a session's history is detected via a keyed stamp on the connection row. The `connections` table itself is **not** chained: deleting a whole session, or editing its row, is evidenced by the chained `connection.opened` / `connection.closed` audit entries rather than caught by a chain walk. |
...
| `DBB_QUERY_STORAGE_RETENTION` (query history and captured rows), `DBB_CONNECTION_RETENTION` (the session ledger) and `DBB_DUMP_RETENTION` (local capture spool) | **A.8.10 Information deletion** | Retention is an explicit, configured duration, and the sweep runs continuously rather than as a manual clean-up. The statement window and the session-ledger window are set independently, so the data that can hold customer content is minimised without discarding the record of who connected. |
...
| [HMAC-chained audit log and query history](/docs/features/audit-chain) with an offline `dbbat audit verify` command; the chain key is derived from `DBB_KEY` and never stored alongside the records | **10.3** — audit logs are protected from destruction and unauthorized modifications; **10.3.2** — log files are protected to prevent modifications | 10.3 asks that logs cannot be altered without detection. Each record is sealed against the one before it, so an insertion, an edit, a deletion or a reordering is caught by the verification — including one performed directly against DBBat's PostgreSQL store. Note the scope precisely for your assessor: this is **detection**, not write-protection; it does not cover rows written before the chain anchor; it seals a statement with its parameters, and a captured result row with its content, rather than the outcome and capture-completeness columns written afterwards; and deleting the *last* statements of a session is detected by a keyed stamp on the connection row. One deletion is evidenced rather than *detected*: the `connections` row is not chained, so removing a whole session — which cascades to its statements and captured rows — leaves its chained `connection.opened` / `connection.closed` audit entries behind for you to reconcile against, and no chain walk reports it. Restricting who can reach the storage database (**10.3.1**) remains yours. |
| `DBB_QUERY_STORAGE_RETENTION` and `DBB_CONNECTION_RETENTION` (both off by default — history is kept indefinitely unless you set them) | **10.5** — audit log history is retained and available for analysis | The default keeps everything, which satisfies 10.5.1's twelve months by construction; set a retention only if a data-minimisation obligation requires it, and set it above twelve months if this database is in PCI scope. |
...
- **Tamper-evidence is detection, not prevention, and it has edges.** The
  ...
  so statements run since that sweep are in the same position. See
  [Tamper-Evident Audit Log](/docs/features/audit-chain) for the detail.
  ...
  all of them. With retention off — the default — no session is excused. Note
  which column decides that: `connected_at` sits on the `connections` row, which
  **is not chained**, so an attacker able to delete the statements can equally
  backdate it to earn the excuse — again, only where a retention is configured.
  If your control requires *prevention* rather than detection, keep shipping the
  logs to a WORM store or a SIEM as well.
- **The connection row is not chained, so a whole-session delete is detectable
  only by comparison.** `queries.connection_id` and `query_rows.query_id`
  cascade, so `DELETE FROM connections WHERE uid = …` removes a session,
  everything it ran and everything it read, and breaks no chain. What it leaves
  behind is the pair of chained `connection.opened` / `connection.closed` entries
  every session writes into the audit log — a table the cascade does not touch
  and retention never reaps — carrying the row's immutable identity (user,
  database, source IP, `connected_at`, instance and run, grant) and, on close,
  `disconnected_at` and the session's sealed query-chain head. The same entries
  are what make an *edited* connection row visible. State this precisely for an
  assessor: the evidence exists and is tamper-evident, but converting it into a
  finding is a reconciliation **you** run between those entries and the surviving
  connection rows — `dbbat audit verify` does not perform it and will not report
  a deleted session as a break.
...
`"verified": true` without walking anything — and its answer is cached, so it
can be up to 60 seconds old: a chain broken moments ago keeps reporting
`"verified": true` until the cached walk expires. The endpoint is a monitoring
signal; the CLI is the point-in-time attestation. Collect the API result as
routine evidence; run the CLI from a trusted binary when the integrity of the
DBBat process itself is in scope, and say which one produced a given artifact.
