...
- **Tamper-evidence is detection, not prevention, and it has edges.** The
  ...
  **before 0.24** carry a head stamp that is a plain copy of the last
  ...
  no key. From 0.24 that stamp is a keyed MAC, like the captured result rows'
  always was. Existing stamps cannot be re-sealed (the key never enters the
  database), so they keep the old format and are counted rather than trusted:
  `dbbat audit verify --queries` reports them as
  `sessions_with_legacy_forgeable_head_stamp`, and the count drains as those
  sessions age out of retention. See
  [Tamper-Evident Audit Log](/docs/features/audit-chain) for the detail; until
  that count reaches zero, treat trailing-deletion detection on the *older* part
  of the query history as covering the careless case rather than the deliberate
  one. If your control requires
