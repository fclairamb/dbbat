...
### `routeTree.gen.ts` Keeps Changing on Branches That Touch No Route

`src/routeTree.gen.ts` is generated by the TanStack Router plugin and committed;
CI fails the frontend job when a build leaves it modified. If it comes back
reordered by a couple of hundred lines after a build, the cause is almost always
a **long-lived `make dev` server**: Node caches ES modules per process, so a Vite
server started before a frontend dependency bump keeps generating with the
*previous* `@tanstack/router-generator`, while cold builds use the installed one.
Generators before 1.167.25 did not sort the route nodes at all, which is where
the reordering comes from.

Restart `make dev`, then re-run `make build-front` and commit whatever that
produces. `vite.config.ts` carries a dev-only guard that warns when the router
codegen packages change under a running server; it cannot fix it for you,
because Vite externalises `node_modules` when it reloads a config and the stale
plugin survives a `server.restart()`.

An mtime-only touch (`git status` shows the file modified but `git diff` is
empty) is a different, harmless thing: the dev server rewrote it with identical
content.
