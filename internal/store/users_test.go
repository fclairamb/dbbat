...
func TestGetUsernamesByIDs(t *testing.T) {
	...
	alice, err := store.CreateUser(ctx, "alice", "hash", []string{RoleConnector})
	if err != nil {
		t.Fatalf("CreateUser(alice) error = %v", err)
	}
	bob, err := store.CreateUser(ctx, "bob", "hash", []string{RoleViewer})
	if err != nil {
		t.Fatalf("CreateUser(bob) error = %v", err)
	}

	t.Run("resolves multiple ids", func(t *testing.T) {
		names, err := store.GetUsernamesByIDs(ctx, []uuid.UUID{alice.UID, bob.UID})
		if err != nil {
			t.Fatalf("GetUsernamesByIDs() error = %v", err)
		}
		if names[alice.UID] != "alice" {
			t.Errorf("names[alice] = %q, want %q", names[alice.UID], "alice")
		}
		if names[bob.UID] != "bob" {
			t.Errorf("names[bob] = %q, want %q", names[bob.UID], "bob")
		}
	})

	t.Run("missing id is absent from the map", func(t *testing.T) {
		missing := uuid.New()
		names, err := store.GetUsernamesByIDs(ctx, []uuid.UUID{alice.UID, missing})
		if err != nil {
			t.Fatalf("GetUsernamesByIDs() error = %v", err)
		}
		if _, ok := names[missing]; ok {
			t.Errorf("expected missing id to be absent, got %q", names[missing])
		}
		if len(names) != 1 {
	...
	t.Run("empty input returns empty map", func(t *testing.T) {
		names, err := store.GetUsernamesByIDs(ctx, nil)
		if err != nil {
			t.Fatalf("GetUsernamesByIDs(nil) error = %v", err)
		}
		if len(names) != 0 {
