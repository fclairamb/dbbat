...
func TestAuthCache_VerifyKey(t *testing.T) {
	...
	// First verification should miss cache
	valid, err := cache.VerifyKey(keyID, apiKey, hash)
	if err != nil {
...
func TestAuthCache_VerifyKey_WrongKey(t *testing.T) {
	...
	valid, err := cache.VerifyKey(keyID, "dbb_wrongkey123456789012345678901", hash)
	if err != nil {
		t.Fatalf("VerifyKey failed: %v", err)
	}
	if valid {
...
func TestAuthCache_VerifyKey_DifferentKeys(t *testing.T) {
	...
	// Verify both keys
	valid, err := cache.VerifyKey("key-1", apiKey1, hash1)
	if err != nil {
		t.Fatalf("VerifyKey failed: %v", err)
	}
	if !valid {
	...
	valid, err = cache.VerifyKey("key-2", apiKey2, hash2)
	if err != nil {
		t.Fatalf("VerifyKey failed: %v", err)
	}
	if !valid {
	...
	_, _ = cache.VerifyKey("key-1", apiKey1, hash1)
	_, _ = cache.VerifyKey("key-2", apiKey2, hash2)
