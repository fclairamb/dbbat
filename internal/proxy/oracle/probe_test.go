...
func TestBareTCPProbeIsQuiet(t *testing.T) {
	...
	srv := NewServer(nil, nil, nil, config.QueryStorageConfig{}, config.DumpConfig{}, slog.New(logs))
	...
	require.Eventually(t, func() bool {
		return logs.has(slog.LevelDebug, "Oracle client disconnected before startup")
...
// sends a partial TNS Connect header and then hangs up is a real failure and
// must keep its ERROR line. The classification this replaced matched on the
// error *text* and demoted anything containing "EOF", `unexpected EOF`
// included, so this case used to be silent.
func TestTruncatedStartupStaysAnError(t *testing.T) {
	...
	// Four bytes of an eight-byte TNS header, then a hang-up.
	_, err = conn.Write([]byte{0x00, 0x3a, 0x00, 0x00})
	require.NoError(t, err)
	require.NoError(t, conn.Close())

	require.Eventually(t, func() bool {
		return logs.has(slog.LevelError, "Oracle session error")
	}, 5*time.Second, 10*time.Millisecond, "a truncated connect packet must stay an error")

	assert.False(t, logs.has(slog.LevelDebug, "Oracle client disconnected before startup"),
