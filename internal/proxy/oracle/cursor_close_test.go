...
func TestDecodeCloseCursors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// testdata/sqlplus_cursor_reexec.pcapng, client frame 9. sqlplus
			// (OCI thick) frames this op in the wide encoding: seq 0x08, then
			// the two-byte pad 0x01 0x09 (0x09 = seq+1, the sequence number of
			// the fetch-rows message stapled right behind this one), then the
			// 8-byte pointer sentinel, then count and id as little-endian
			// uint32s.
			name: "OCI wide encoding, sqlplus frame 9",
			payload: []byte{
		...
		{
			// testdata/sqlplus_cursor_reexec.pcapng, client frame 12: closes
			// cursor 4, with the piggyback execute (func 0x03 sub 0x5e) for the
			// next run stapled behind it in the same packet — its header
			// carries the identical wide two-byte pad (seq 0x0e, pad 0x01 0x0f).
			name: "OCI wide encoding, sqlplus frame 12",
			payload: []byte{
				0x11, 0x69, 0x0d, 0x01, 0x0e, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0x01, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00,
				0x03, 0x5e, 0x0e, 0x01, 0x0f, 0x61, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			},
			want: []uint16{4},
		},
		{
			// testdata/sqlplus_cursor_reexec.pcapng, client frame 14: closes
			// cursor 3.
			name: "OCI wide encoding, sqlplus frame 14",
			payload: []byte{
				0x11, 0x69, 0x10, 0x01, 0x11, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0x01, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
			},
			want: []uint16{3},
		},
		{
			// testdata/sqlplus_cursor_reexec.pcapng, client frame 16: closes
			// cursor 2 — the same id frame 9 opened, showing Oracle recycling it
			// within one session.
			name: "OCI wide encoding, sqlplus frame 16",
			payload: []byte{
				0x11, 0x69, 0x13, 0x01, 0x14, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
			},
			want: []uint16{2},
		},
...
func TestDecodeCloseCursorsRejectsWhatItCannotRead(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The wide two-byte pad without a matching sentinel behind it: byte
			// 3 is the pointer-flag value (0x01) the thin path also checks for,
			// but what follows is not the 8-byte fe-run, so this must fall
			// through to (and fail) the thin walk rather than being read as
			// wide.
			name: "wide-shaped pad but no sentinel",
			payload: []byte{
				0x11, 0x69, 0x08, 0x01, 0x09, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00,
				0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
			},
		},
		{
			// A sentinel present, but the two-byte pad doesn't name this
			// header's own seq+1 — must not be read as wide.
			name: "sentinel present but pad byte wrong",
			payload: []byte{
				0x11, 0x69, 0x08, 0x01, 0x05, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
			},
		},
		{
			// A wide-shaped header (valid pad + sentinel) whose count would
			// exceed the plausible batch size — the wide path's own guard.
			name: "OCI wide encoding, implausible count",
			payload: []byte{
				0x11, 0x69, 0x08, 0x01, 0x09, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0x00, 0x00, 0x00, 0x10,
			},
		...
		{
			// A wide-shaped header whose id is zero (allocate, not close) —
			// the wide path's guard mirroring the thin one.
			name: "OCI wide encoding, cursor id zero",
			payload: []byte{
				0x11, 0x69, 0x08, 0x01, 0x09, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
