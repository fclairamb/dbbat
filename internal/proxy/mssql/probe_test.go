...
// proxy treats it as an expected outcome: a DEBUG line, nothing louder. The
// probe was already silent here (isExpectedDisconnect swallowed the EOF); what
// this pins is that it stays observable at DEBUG and never climbs above it.
func TestBareTCPProbeIsQuiet(t *testing.T) {
	...
	srv, err := NewServer(nil, nil, config.QueryStorageConfig{}, config.DumpConfig{}, nil,
		config.MSSQLConfig{TLS: config.TLSConfig{Disable: true}}, slog.New(logs))
	...
	require.Eventually(t, func() bool {
		return logs.has(slog.LevelDebug, "MSSQL client disconnected before startup")
...
// TestTruncatedStartupStaysLoud is the other half of the rule: a client that
// starts a TDS packet and then hangs up is a real failure and keeps its
// "session ended" line. Demoting it too would hide truncated clients.
func TestTruncatedStartupStaysLoud(t *testing.T) {
	t.Parallel()
	...
	// Four bytes of an eight-byte TDS packet header, then a hang-up.
	_, err = conn.Write([]byte{0x12, 0x01, 0x00, 0x2f})
	require.NoError(t, err)
	require.NoError(t, conn.Close())

	require.Eventually(t, func() bool {
		return logs.has(slog.LevelInfo, "MSSQL session ended")
	}, 5*time.Second, 10*time.Millisecond, "a truncated PRELOGIN must stay reported")

	assert.False(t, logs.has(slog.LevelDebug, "MSSQL client disconnected before startup"),
