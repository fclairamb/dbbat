...
// proxy treats it as an expected outcome: a DEBUG line, nothing louder. On
// this protocol that replaces the INFO "MongoDB session ended" every probe used
// to produce.
func TestBareTCPProbeIsQuiet(t *testing.T) {
	...
	srv, err := NewServer(nil, nil, config.QueryStorageConfig{}, config.DumpConfig{}, nil,
		config.MongoConfig{TLS: config.TLSConfig{Disable: true}}, slog.New(logs))
	...
	require.Eventually(t, func() bool {
		return logs.has(slog.LevelDebug, "MongoDB client disconnected before startup")
...
// starts a message and then hangs up is a real failure and keeps its
...
func TestTruncatedStartupStaysLoud(t *testing.T) {
	...
	// One byte — enough for the transport peek to succeed and commit the
	// session to plaintext — then a hang-up mid-message.
	_, err = conn.Write([]byte{0x25})
	require.NoError(t, err)
	require.NoError(t, conn.Close())

	require.Eventually(t, func() bool {
		return logs.has(slog.LevelInfo, "MongoDB session ended")
	}, 5*time.Second, 10*time.Millisecond, "a truncated startup must stay reported")

	assert.False(t, logs.has(slog.LevelDebug, "MongoDB client disconnected before startup"),
