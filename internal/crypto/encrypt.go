...
// CLIAuthAAD returns the AAD for encrypting a freshly minted API key while it
// sits in a pending CLI authorization request. This binds the ciphertext to
// the specific request UID, preventing a key transplant between requests.
func CLIAuthAAD(requestUID string) []byte {
	return []byte(fmt.Sprintf("cliauth:%s", requestUID))
}
