...
- **Bind values (parameterized queries)**: Bind values are captured only from the legacy `OALL8` execute path (`decodeBindValues`), and even there a NUMBER bind is stored as hex rather than its decimal value. Modern clients (go-ora, and the v315+ piggyback execute path generally) send the statement as a **piggyback exec** (func `0x03` sub `0x5e`); `decodePiggybackExecSQL` extracts the SQL but **not** the binds, so those parameters are not captured. The binds are present at the tail of that message — bind-definition records (the same `ParameterInfo`/describe-record format `parseColumnDescribes` handles) followed by length-prefixed values, e.g. `02 c1 2b` (NUMBER 42) and `05 68 65 6c 6c 6f` ("hello") for `SELECT :1 || '-' || :2 …`, with the bind count near offset `0x1a` of the payload. Type-aware extraction is a planned follow-up; ground-truth fixture: `testdata/go_ora_binds.dbbat-dump`.
