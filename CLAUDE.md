...
| `DBB_AUTH_DEFAULT_ROLE` | Role an auto-provisioned account starts with, and the floor `DBB_OIDC_ROLE_MAPPING` never digs below (default: `connector`). Validated against the known roles at startup — a typo is a **startup failure**, not a user silently provisioned into a role nothing recognizes. Legacy alias `DBB_SLACK_AUTH_DEFAULT_ROLE`, same precedence rule | No |
